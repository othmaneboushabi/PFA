...
class OrchestratorService:
    ...
    async def process_audio(
        ...
        logger.success(f"Transcription OK : {len(transcription_text)} caractères")

        # Etape 2 : NER + Translation en parallèle
        tasks = []
        if extract_entities:
            logger.info("[2/3] Extraction entités (NER)...")
        ...
        if tasks:
            ...
            if extract_entities:
                ...
                logger.success(f"NER OK : {len(result['entities'])} entités")
            if translate:
                translation_idx = 1 if extract_entities else 0
                result["translation"] = results_parallel[translation_idx]
                logger.success(f"Traduction OK")

        # Etape 3 : Enrichissement glossaire
        if enrich_glossary and translate and "translation" in result and source_lang != "ar":
            logger.info("Enrichissement glossaires...")
            result["glossary_matches"] = await self._enrich_with_glossary(
                transcription_text, source_lang, target_lang,
            )
            logger.success(f"Glossaire : {len(result['glossary_matches'])} termes")
    ...
    async def _translate_text(self, text: str, src_lang: str, tgt_lang: str) -> dict:
        return self.translation_service.translate(text=text, src_lang=src_lang, tgt_lang=tgt_lang)

    async def _enrich_with_glossary(self, text: str, src_lang: str, tgt_lang: str) -> list:
        words = set(text.split())
        matches = []
        for word in words:
            results = await self.glossary_service.search_term(
                term=word, source_lang=src_lang, target_lang=tgt_lang, fuzzy_threshold=90,
            )
            if results:
